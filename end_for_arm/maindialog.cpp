...
void MainDialog::on_pushButton_2_clicked()
{
    if(!state2)
    {
        if(led_fd = init_led())                                //打开LED显示设备
        {
            change_led(10, led_fd);                          //初始化LED显示
...
void MainDialog::showTime()
{
    ...
    QString text = time.toString("hh:mm");                  //格式化显示  小时:分钟
    if ((time.second() % 2) == 0)                           
        text[2] = ' ';
    ui->lcdNumber->display(text);                           //显示
}

//真*电机转速
void MainDialog::on_horizontalSlider_valueChanged(int value) //当滑块值改变的槽函数
{
...
void MainDialog::update()
{
	...
	t = get_value(2)*1024-512;                      //获取ad2的值（-512~512）
...
void MainDialog::on_pushButton_4_clicked()             //
{
    ...
    {
        QIcon icon;
        icon.addFile(":/images/zoomnormal.png");      //图片换x
